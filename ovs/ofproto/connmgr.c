                       ...
                       const struct ofproto_async_msg *am)
{
    ...
    //    check = false;
       ...
       //bloom
        // char str[100];
        //     sprintf(str,"%x,%x,%d",value,dst,header.tp_src);
        // fp=fopen("/home/iot_team/debug.log","a");
        ...
        //         fp=fopen("/home/iot_team/debug1.log","a");
        //         fprintf(fp,"notpass\n");
        //         // check = false;
        //         fclose(fp);
        //         return;
    ...
    //    uint32_t ack = ntohl(get_16aligned_be32(&tcp->tcp_ack));
    //    fp=fopen("/home/iot_team/tcp_ack.log","a");
    //     fprintf(fp, "%d, %d\n", &tcp->tcp_seq.hi, &tcp->tcp_seq.lo);
    //     fclose(fp);
        // struct tcp_reader *reader;
        // struct dp_packet *packet;
        // tcp_reader_run(reader, &header, packet);
        // const char *l7 = dp_packet_get_tcp_payload(&pkt);
        // tcp = dp_packet_l4(&pkt);
        // uint64_t seq = ntohl(get_16aligned_be32(&tcp->tcp_seq));
        // // uint64_t ack = ntohl(get_16aligned_be32(&tcp->tcp_ack));
        // fp=fopen("/home/iot_team/seq.log","a");
        // fprintf(fp, "%+u\n", seq);
        // fclose(fp);
        message = malloc(STR_LENGTH * sizeof(char));
        for(int i=0; i<10; i++)
        ...
        if (Syn_ack == true)
        {
            ...
            // _exit(1);
            // char bar[1000]="/home/iot_team/Desktop/OVS/bash.sh ";
            char bar[1000] = "echo '1' | sudo -S python /home/iot_team/SYN-ACK.py ";
            strcat(bar, argv1);
            strcat(bar, " ");
            strcat(bar, argv2);
            strcat(bar, " ");
    ...
    else if(header.tcp_flags == 1024)  //ban tin RST
    {
         const char *l7 = dp_packet_get_tcp_payload(&pkt);
        tcp = dp_packet_l4(&pkt);
        uint64_t seq = ntohl(get_16aligned_be32(&tcp->tcp_seq));
        // uint32_t ack = ntohl(get_16aligned_be32(&tcp->tcp_ack));
        // fp=fopen("/home/iot_team/seq_ack.log","a");
        // fprintf(fp, "%d\n", seq - 1000);
        // fclose(fp);
        // fp=fopen("/home/iot_team/TCP_RST.log","a");
        // fprintf(fp, "%s\n", "RST Received");
        // fclose(fp);
        // fp=fopen("/home/iot_team/debug.log","a");
        struct tcp_header *tcp;
        // fp=fopen("/home/iot_team/TCP_RST.log","a");
        // fprintf(fp, "%d, %d\n", &tcp->tcp_ack.hi, &tcp->tcp_ack.lo);
        // fclose(fp);
        message = malloc(STR_LENGTH * sizeof(char));
        for(int i=0; i<10; i++)
        {
    ...
    else if (htons(header.tcp_flags) == 16)
     {
        ...
        // fprintf(fp,"index = %d = %c, %d = %c, %d = %c, %d = %c, %d = %c, %d = %c, %d = %c, %d = %c, %d = %c, %d = %c \n", 
        //         hash_value[0], mgr->T[hash_value[0]], 
        //         hash_value[1], mgr->T[hash_value[1]], 
        //         hash_value[2], mgr->T[hash_value[2]], 
        //         hash_value[3], mgr->T[hash_value[3]],
        //         hash_value[4], mgr->T[hash_value[4]], 
        //         hash_value[5], mgr->T[hash_value[5]], 
        //         hash_value[6], mgr->T[hash_value[6]], 
        //         hash_value[7], mgr->T[hash_value[7]],
        //         hash_value[8], mgr->T[hash_value[8]], 
        //         hash_value[9], mgr->T[hash_value[9]]);
        // fclose(fp);
     }
    else check = true;
